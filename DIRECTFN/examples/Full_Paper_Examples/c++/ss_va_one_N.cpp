...
void  ss_va_one_N() noexcept {
    ...
    Q.set_points(r1, r4, r3, r2, r5, r6, r7);

	unique_ptr<Quadrilateral_VA<QuadrilateralKernel_PlanarVectorSS>> up_quad_va(new Quadrilateral_VA<QuadrilateralKernel_PlanarVectorSS>());
    ...
    for (size_t i = 0; i < up_quad_va->kernel_size(); ++i) {
        cout << setprecision(20) << up_quad_va->Iss_arr(i) << endl;
    }
	cout << endl;
	*/

	std::ofstream myfile;
	myfile.open("Results_ss_va_one_N.txt");
	...
	for (int N = 1; N <= Counter; N++)
	{
		...
		// Setting Gaussian orders
		up_quad_va->set_Gaussian_orders_4(N, 20, 20, 20);
		up_quad_va->calc_Iss();
		I_quad = up_quad_va->Iss();
		...
		// Setting Gaussian orders
		up_quad_va->set_Gaussian_orders_4(20, N, 20, 20);
		up_quad_va->calc_Iss();
		I_quad = up_quad_va->Iss();
		...
		// Setting Gaussian orders
		up_quad_va->set_Gaussian_orders_4(20, 20, N, 20);
		up_quad_va->calc_Iss();
		I_quad = up_quad_va->Iss();
		...
		// Setting Gaussian orders
		up_quad_va->set_Gaussian_orders_4(20, 20, 20, N);
		up_quad_va->calc_Iss();
		I_quad = up_quad_va->Iss();
