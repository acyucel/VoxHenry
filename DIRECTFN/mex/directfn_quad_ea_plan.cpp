...
{  
  ...
  const string btt_type = mxArrayToString(prhs[11]);
  int len;
  int status = -1;
  ...
  if (btt_type == string("Constant")) {
	  ...
	  status = directfn_quad_ea_plan<QuadrilateralKernel_PlanarScalar>(r1, r2, r3, r4, r5, r6, N1, N2, N3, N4, k0, I_DE.get());
  }
  else if (btt_type == string("Vector_WS")) {
	  len = QuadrilateralKernel_PlanarVectorWS::constexpr_size();
	  I_DE.reset(new dcomplex[len]);
	  status = directfn_quad_ea_plan<QuadrilateralKernel_PlanarVectorWS>(r1, r2, r3, r4, r5, r6, N1, N2, N3, N4, k0, I_DE.get());
  }
  else if (btt_type == string("Vector_SS")) {
	  len = QuadrilateralKernel_PlanarVectorSS::constexpr_size();
	  I_DE.reset(new dcomplex[len]);
	  status = directfn_quad_ea_plan<QuadrilateralKernel_PlanarVectorSS>(r1, r2, r3, r4, r5, r6, N1, N2, N3, N4, k0, I_DE.get());
  }
