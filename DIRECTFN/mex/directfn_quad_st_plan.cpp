...
{  
  ...
  if (btt_type == string("Constant")) {
	  ...
	  status = directfn_quad_st_plan<QuadrilateralKernel_PlanarScalar>(r1, r2, r3, r4, N1, N2, N3, N4, k0, I_DE.get());
  }
  else if (btt_type == string("Vector_WS")) {
	  len = QuadrilateralKernel_PlanarVectorWS::constexpr_size();
	  I_DE.reset(new dcomplex[len]);
	  status = directfn_quad_st_plan<QuadrilateralKernel_PlanarVectorWS>(r1, r2, r3, r4, N1, N2, N3, N4, k0, I_DE.get());
  }
