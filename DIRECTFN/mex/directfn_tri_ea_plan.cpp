...
{  
  ...
  const string btt_type = mxArrayToString(prhs[9]);
  int status = -1;
  int len;
  unique_ptr<dcomplex[]> I_DE(nullptr);
  
  if (btt_type == string("Constant")) {
	  len = TriangularKernel_Constant_EA::constexpr_size();
	  I_DE.reset(new dcomplex[len]);
	  status = directfn_tri_ea_plan<TriangularKernel_Constant_EA>(r1, r2, r3, r4, N1, N2, N3, N4, k0, I_DE.get());
  }
  else if (btt_type == string("RWG_WS")) {
	  len = TriangularKernel_RWG_WS::constexpr_size();
	  I_DE.reset(new dcomplex[len]);
	  status = directfn_tri_ea_plan<TriangularKernel_RWG_WS>(r1, r2, r3, r4, N1, N2, N3, N4, k0, I_DE.get());
  }
  else if (btt_type == string("RWG_SS")) {
	  len = TriangularKernel_RWG_SS::constexpr_size();
	  I_DE.reset(new dcomplex[len]);
	  status = directfn_tri_ea_plan<TriangularKernel_RWG_SS>(r1, r2, r3, r4, N1, N2, N3, N4, k0, I_DE.get());
  }
  else if (btt_type == string("nxRWG_SS")) {
	  len = TriangularKernel_nxRWG_SS::constexpr_size();
	  I_DE.reset(new dcomplex[len]);
	  status = directfn_tri_ea_plan<TriangularKernel_nxRWG_SS>(r1, r2, r3, r4, N1, N2, N3, N4, k0, I_DE.get());
  }
