...
hold on
h=loglog(fH_data(:,1),real(fH_data(:,2)),'ro'); set(h,'LineWidth',2);
...
hold on
h=loglog(fH_data(:,1),1e9*imag(fH_data(:,2)),'ro'); set(h,'LineWidth',2);
legend('VoxHenry','FastHenry');
axis tight;grid on;xlabel('Frequency (Hz)');ylabel('Inductance (nH)');
set(gca,'YTick',[0.21 0.22 0.23]); ylim([0.21 0.23]);
set(gca,'XTick',[10^0 10^2 10^4 10^6 10^8 10^10])
set(gca,'FontSize',24);set(gca,'FontName','Times New Roman');
set(gca,'LineWidth',1); grid minor;
...
load('results_numex4_square_coil/data_CPU_timings.mat');
disp('-----------------------------------------------------')
disp('CPU times ::: ')
...
load('results_numex4_square_coil/data_curr_plot.mat')
plot_option=1;
[L,M,N] = size(Mc);
...
print('results_numex4_square_coil/curr_dist',  '-dpng', '-r300')
