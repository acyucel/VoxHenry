...
load('results_numex2_wire/data_R_jL_mat.mat');
disp('-----------------------------------------------------')
disp('R+jL matrices ::: ')
...
len_wire=50e-6; rad_wire=5e-6;sigma_e=5.8e7;
res_analytic=zeros(num_freq,1);ind_analytic=zeros(num_freq,1);
for freq_no=1:num_freq
...
subplot(2,1,1)
set(gca,'FontSize',24); set(gca,'FontName','Times New Roman');
h=loglog(freq_all,real(squeeze(R_jL_mat)),'b-'); set(h,'LineWidth',2);
hold on
h=loglog(freq_all,res_analytic,'ro'); set(h,'LineWidth',2);
legend('VoxHenry','Analytical Formula');
axis tight;grid on;xlabel('Frequency');ylabel('Resistance');
ylim([0.01 0.045]); set(gca,'YTick',[0.01 0.02 0.03 0.04])
set(gca,'FontSize',24);set(gca,'FontName','Times New Roman');
...
subplot(2,1,2)
set(gca,'FontSize',24); set(gca,'FontName','Times New Roman');
h=loglog(freq_all,imag(squeeze(R_jL_mat)),'b-'); set(h,'LineWidth',2);
hold on
...
load('results_numex2_wire/data_CPU_timings.mat');
disp('-----------------------------------------------------')
disp('CPU times ::: ')
...
load('results_numex2_wire/data_curr_plot.mat')
plot_option=1;
[L,M,N] = size(Mc);
...
print('results_numex2_wire/curr_dist',  '-dpng', '-r300')
